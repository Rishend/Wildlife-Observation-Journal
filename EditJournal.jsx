...
const EditJournal = ({journalData}) => {
  ...
  return (
    <div>
        ...
        <button className=' form-control btn btn-primary' onClick={addText}>Add Text</button>
        
        <label>Upload Video</label>
        <input type="file" className='form-control btn btn-secondary' onChange={(e) => { uploadFile(e, addVideo) }} />

        <label htmlFor="">Upload Image</label>
        <input type="file" className='form-control btn btn-secondary ' onChange={(e) => { uploadFile(e, addImage) }} />
        
        <hr />

        <button className='btn btn-success mt-4 rounded-pill' onClick={updateJournal}>Save Changes</button>
